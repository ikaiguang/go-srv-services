...
func NewHTTPServer(engineHandler setup.Engine) (srv *http.Server, err error) {
	...
	// 响应
	opts = append(opts, http.ResponseEncoder(apppkg2.ResponseEncoder))
	opts = append(opts, http.ErrorEncoder(apppkg2.ErrorEncoder))
	...
	middlewareSlice = append(middlewareSlice, apppkg2.ServerLog(
		middleLogger,
