...
// NewRedisTokenRepo ...
func NewRedisTokenRepo(redisCC redis.UniversalClient, opts ...Option) AuthTokenRepo {
	o := &options{}
	for i := range opts {
		opts[i](o)
	}
	if o.authConfig == nil {
		o.authConfig = &confv1.Business_Auth{}
	}
...
// SigningSecret 签名密码
func (s *redisToken) SigningSecret(ctx context.Context, tokenType commonv1.TokenTypeEnum_TokenType, passwordHash string) string {
	if passwordHash == "" {
...
// SaveCacheData 存储缓存
func (s *redisToken) SaveCacheData(ctx context.Context, authClaims *authutil.Claims, authCache *commonv1.Auth) error {
	var (
...
// SetTokenType 设置令牌类型
func (s *redisToken) SetTokenType(operation string, tokenType commonv1.TokenTypeEnum_TokenType) {
	// 写锁
...
// GetTokenType 获取令牌类型
// 默认值 : 0 : commonv1.TokenTypeEnum_DEFAULT
func (s *redisToken) GetTokenType(operation string) commonv1.TokenTypeEnum_TokenType {
	// 暂不加锁：不考虑动态增加令牌，并发读
...
func (s *redisToken) JWTKeyFunc() authutil.KeyFunc {
	...
	authKeyFunc := func(ctx context.Context) jwt.Keyfunc {
		// 令牌类型
		tokenType := commonv1.TokenTypeEnum_DEFAULT
...
func (s *redisToken) ValidateFunc() authutil.ValidateFunc {
	validator := func(ctx context.Context, jwtToken *jwt.Token) error {
		...
		case commonv1.StatusEnum_DISABLE, commonv1.StatusEnum_BLACKLIST, commonv1.StatusEnum_DELETED:
			// 禁用、黑名单、已删除
...
func (s *redisToken) validateAuthInfo(authClaims *authutil.Claims, authInfo *commonv1.Auth) (err error) {
	// 无限制
	switch authInfo.Payload.Lt {
	case commonv1.LimitTypeEnum_UNLIMITED:
		return err
	case commonv1.LimitTypeEnum_ONLY_ONE:
		// 仅一个
		if authClaims.Payload.St.AsTime().UnixNano() != authInfo.Payload.St.AsTime().UnixNano() {
			return authutil.ErrLoginLimit
		}
	case commonv1.LimitTypeEnum_PLATFORM_ONE:
		// 平台限制一个
...
func (s *redisToken) getCacheData(ctx context.Context, cacheKey string) (*commonv1.Auth, error) {
...
func (s *redisToken) saveCacheData(ctx context.Context, cacheKey string, authInfo *commonv1.Auth, expiration time.Duration) error {
